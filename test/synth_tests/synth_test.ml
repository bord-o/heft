...
let%expect_test "synth goal setup" =
  ...
  let proof = with_term [%term 0n] exists >> intro >> simp ~with_asms:true in
...
let%expect_test "synth goal setup full" =
  ...
  let proof =
    with_term zero exists >> with_term cons_witness exists >> intros >> auto_dfs
  in
  run_proof goal proof;
  [%expect
    {|
...
let%expect_test "synth goal enumerate" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term 5 (with_info_trace exists))
      >> try_ (with_synthetic_term 5 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
...
let%expect_test "synth append" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term 2 (with_info_trace exists))
      >> try_ (with_synthetic_term 5 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
...
let%expect_test "synth reverse" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra 2 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra 6 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
  [%expect
...
let%expect_test "synth mult" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:extras 3 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra:extras 4 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
  [%expect
...
let%expect_test "synth sum" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:extras 5 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra:extras 5 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
...
let%expect_test "synth sum via make_synthesis_goal" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:plus_extra 5 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra:plus_extra 5 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
...
let%expect_test "synth append via make_synthesis_goal" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term 2 (with_info_trace exists))
      >> try_ (with_synthetic_term 6 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
  [%expect
...
let%expect_test "synth mult via make_synthesis_goal" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:plus_extra 3 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra:plus_extra 5 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
...
let%expect_test "synth reverse via make_synthesis_goal" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:append_extra 3 (with_info_trace exists))
      >> try_
           (with_synthetic_term ~extra:append_extra 6 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
  [%expect
...
let%expect_test "synth replicate via make_synthesis_goal" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term 2 (with_info_trace exists))
      >> try_ (with_synthetic_term 4 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
  [%expect
...
(*       (try_ *)
(*          (with_info_trace *)
(*             (with_synthetic_term ~extra:extras 3 (with_info_trace exists))) *)
(*       >> try_ *)
(*            (with_info_trace *)
(*               (with_synthetic_term ~extra:extras 7 (with_info_trace exists))) *)
(*       >> intros >> auto_dfs) *)
(*   in *)
(*   run_proof goal proof; *)
(*   [%expect {| |}] *)
...
let%expect_test "synth list_sum_pairs via make_synthesis_goal" =
  ...
  let proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:extras 3 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra:extras 6 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  run_proof goal proof;
  [%expect
...
let%expect_test "synth insert via make_synthesis_goal" =
  ...
  let _proof =
    with_best_first
      (try_ (with_synthetic_term ~extra:extras 6 (with_info_trace exists))
      >> try_ (with_synthetic_term ~extra:extras 8 (with_info_trace exists))
      >> intros >> auto_dfs)
  in
  ();
  (* run_proof goal proof; *)
...
(* Bend2 author claimed that this synthesis problem wasn't feasible with other synth systems, he is very wrong *)
(* let _ = *)
(*   (* f : nat -> nat, nat pair -> nat, nat pair*) *)
(*   let* func_type = *)
(*     [%term (f : nat -> (nat, nat) pair -> (nat, nat) pair)] |> type_of_term *)
(*   in *)
(*   let* fst_ty = [%term (fst_mono : (nat, nat) pair -> nat)] |> type_of_term in *)
(*   let* snd_ty = [%term (snd_mono : (nat, nat) pair -> nat)] |> type_of_term in *)
(*   let* pred_ty = [%term pred] |> type_of_term in *)
(*   (* let* plus_ty = [%term plus] |> type_of_term in *) *)
(*   (* let* sub_ty = [%term sub] |> type_of_term in *) *)
(*   let extras = *)
(*     [ *)
(*       ("fst", fst_ty); *)
(*       ("snd", snd_ty); *)
...
(*   let test_cases = *)
(*     [ *)
(*       ([ [%term 3n]; [%term Pair (5n, 5n)] ], [%term Pair (8n, 2n)]); *)
(*       ([ [%term 2n]; [%term Pair (1n, 4n)] ], [%term Pair (3n, 2n)]); *)
(*       ([ [%term 1n]; [%term Pair (5n, 2n)] ], [%term Pair (6n, 1n)]); *)
(*     ] *)
(*   in *)
(*   let goal_tm = make_synthesis_goal ~func_type ~test_cases in *)
(*   let goal = ([], goal_tm) in *)
(*   let proof = *)
(*     with_best_first *)
(*       (try_ (with_info_trace @@ with_synthetic_term ~extra:extras 2 exists) *)
(*       >> try_ *)
(*            (with_info_trace *)
(*            @@ with_synthetic_term ~extra:extras 6 (with_info_trace exists)) *)
(*            >> with_dfs' auto) *)
(*   in *)
(*   run_proof goal proof; *)
(*   Ok () *)
