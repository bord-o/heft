...
let%expect_test "positivity_nested_left" =
  let constructors = [
    { name = "Bad"; arg_types = [mk_fun_ty (mk_fun_ty nat_ty int_ty) bool_ty] }
  ] in
  print_bool_result (check_positivity "nat" constructors);
  [%expect {| false |}]
...
let%expect_test "positivity_deeply_nested_ok" =
  let constructors = [
    { name = "Deep"; arg_types = [mk_fun_ty int_ty (mk_fun_ty bool_ty nat_ty)] }
  ] in
  print_bool_result (check_positivity "nat" constructors);
  [%expect {| true |}]
