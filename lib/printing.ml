...
let rec pretty_print_hol_term ?(with_type = false) ?(parent_prec = PrecImp) term =
  ...
  | App (App (Const ("=", _), l), r) ->
      Format.sprintf "%s = %s" (aux ~parent_prec:PrecApp l) (aux ~parent_prec:PrecApp r)
  
  | App (Const ("~", _), p) ->
      Format.sprintf "¬%s" (aux ~parent_prec:PrecAtom p)
  
  | App (App (Const ("/\\", _), p), q) ->
      Format.sprintf "%s ∧ %s" (aux ~parent_prec:PrecApp p) (aux ~parent_prec:PrecApp q)
  
  | App (App (Const ("\\/", _), p), q) ->
      Format.sprintf "%s ∨ %s" (aux ~parent_prec:PrecApp p) (aux ~parent_prec:PrecApp q)
  
  | App (App (Const ("==>", _), p), q) ->
      wrap (Format.sprintf "%s ==> %s" (aux ~parent_prec:PrecImp p) (aux ~parent_prec:PrecImp q))
  
  | App (Const ("!", _), Lam (v, body)) ->
      wrap (Format.sprintf "∀%s. %s" (aux ~parent_prec:PrecAtom v) (aux ~parent_prec:PrecQuant body))
  ...
  | Lam (Var (name, ty), body) ->
      ...
      wrap (Format.sprintf "λ%s%s. %s" name ty_str (aux ~parent_prec:PrecQuant body))
  
  | Lam (bind, body) ->
      wrap (Format.sprintf "λ%s. %s" (aux ~parent_prec:PrecAtom bind) (aux ~parent_prec:PrecQuant body))
...
let pretty_print_thm ?(with_type = false) (Sequent (assm, concl)) =
  ...
  | _ ->
      let assms =
        ...
        |> String.concat "\n"
      in
      Format.sprintf "%s\n%s\n%s" assms bar (pretty_print_hol_term ~with_type concl)
